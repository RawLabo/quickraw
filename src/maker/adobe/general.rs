...
use std::cmp;
use super::{super::utility::*, *, decode_utility::ljpeg::LjpegDecompressor};
...
impl RawDecoder for AdobeGeneral {
    ...
    fn get_task(only_thumbnail: bool, _model: String) -> ExifTask {
        if only_thumbnail {
            ...
            let main_ifd_task = create_rule![template {
                0x0100 / width
                0x0101 / height
                0x0102 : u16 / bps
    ...
    fn get_crop(&self) -> Option<Crop> {
        if let (Ok(crop_origin), Ok(crop_size)) = (self.info.u8a4("crop_origin"), self.info.u8a4("crop_size")) {
    ...
    fn inner_pre_process(&self, buffer: &[u8]) -> Result<Vec<u16>, DecodingError> {
        ...
        let image: Vec<u16> = match compression {
            ...
            7 => {
                ...
                to_image!(load_compressed(buffer, width, height, tile_offsets, tile_width, tile_len, self.info.is_le).iter())
            }
            _ => {
                unimplemented!()
            }
...
fn load_compressed(
    ...
    out.chunks_mut(width * tile_len).enumerate().for_each(|(row, strip)| {
        for col in 0..coltiles {
            let offset = (&buffer[tile_offsets..]).u32(is_le,(row * coltiles + col) * 4) as usize;
