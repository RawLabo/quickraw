...
use super::{
    super::utility::*, decode_utility::bit_pump::*, decode_utility::byte_stream::*, decode_utility::huffman::*,
    decode_utility::lookup_table::*, *,
};

impl RawDecoder for NikonGeneral {
    ...
    fn inner_pre_process(&self, buffer: &[u8]) -> Result<Vec<u16>, DecodingError> {
        ...
        let image: Vec<u16> = if width * height * 3 == strip_len {
            ...
            match compression {
                ...
                0x8799 => to_image!(load_raw(buf, color_data, self.info.is_le, bps, width, height)?.iter()),
                _ => unimplemented!(),
            }
        };
...
fn load_raw_yuv2(src: &[u8], wb_r: f64, wb_b: f64, width: usize, height: usize) -> Vec<u16> {
    ...
    out.chunks_mut(width * 3).enumerate().for_each(|(row, out)| {
